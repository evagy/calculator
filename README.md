...
* 支持新建变量时使用已有方法及常量
* 支持三角函数的弧度与角度切换
* 支持输入错误时自定义报错(支持不完善,后续会加强)

如果想要了解详细，请浏览接下来的[使用手册](#用户手册)及[开发者手册](#开发者手册)。
...
在上面内置方法表中，你可能会发现一个名叫```HCF```的求最小公倍数的方法，那你可能会问，有没有相应的求最小公倍数的方法呢？答案是并没有，但是借助可以创建自定义方法，如果设置其方法名为```gcd```, 设置其方法体为```a*b/HCF(a,b)```,就可以使用gcd方法来直接求最小公倍数了。
...
* 允许方法与括号之间、数值与运算符之间存在不影响计算的空格
* 允许幂运算符^连续使用(如: 2^3^4)和嵌套(如:(2+1)^3)
* 更精准的报错系统，应该精确到错误的列数，而不仅仅是只有错误信息
