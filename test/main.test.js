...
describe('优先级测试', () => {
	...
	it('float型加减乘除优先级', () => {
		equal(ca('1.5+1.5*2'), 4.5);
		equal(ca('2.5/1.25+3.5*2'), 9);
	});
	it('加减乘除余幂优先级', () => {
		equal(ca('10*5^3%2+4/2'), 10*Math.pow(5, 3)%2+4/2);
	});
	it('余幂优先级', () => {
		equal(ca('2%2^5'), 2%Math.pow(2, 5));
	});
	it('方法与加减乘除优先级', () => {
		equal(ca('20 + sin(30) - 10 / 5'), 20 + Math.sin(30/180*Math.PI) - 10 / 5);
	});
	it('括号与加减乘除优先级', () => {
		equal(ca('1+(1+1)*5'), 1+(1+1)*5);
	});
	it('括号与加减乘除优先级', () => {
		equal(ca('1+(1+1)*5'), 1+(1+1)*5);
	});
	it('综合优先级', () => {
		equal(ca('3/2 - 1+(1+1)*5^2 + (sin(30)*5) + pow(1+1, 1+1)'), 3/2 - 1+(1+1)*Math.pow(5, 2) + (Math.sin(30/180*Math.PI)*5) + Math.pow(1+1, 1+1));
	});	
});
